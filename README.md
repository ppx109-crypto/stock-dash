...
`.env.example`을 `.env`로 복사하고 `APP_PASSWORD`에 나만의 긴 비밀번호를 입력한 뒤 재실행하세요. 비밀번호를 입력하면 종목 보관함이 열립니다.

`APP_PASSWORD`를 비워 두면 잠금 화면 없이 바로 열리고, 종목 보관함도 그대로 사용합니다. 이 경우 주소를 아는 사람은 누구나 관심종목과 투자일지를 볼 수 있으므로, 공개 주소로 쓸 때는 비밀번호를 두거나 호스팅 서비스의 비공개 접근 제어를 함께 쓰세요.
